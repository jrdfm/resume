...
let ctdgts n = int_of_float (floor (log10 (float_of_int n) +. 1.))
(* if (Str.string_match re_intt s pos) then
       let token = Str.matched_string s in
         (Tok_Int (int_of_string token))::(tok ((pos + ctdgts (int_of_string token)) - 1) s)*)
...
let rec ctdgts n = if n / 10 = 0 then 1 else 1 + (ctdgts n / 10)
...
let tokenize input =
  let rec tok pos s =
    if pos >= String.length s then []
      ...
      else if token = "fun" then Tok_Fun :: tok (pos + length token) s
      else if token = "rec" then Tok_Rec :: tok (pos + length token) s
      else if token = "def" then Tok_Def :: tok (pos + length token) s
      else if token = "let" then Tok_Let :: tok (pos + length token) s
      else if token = "not" then Tok_Not :: tok (pos + length token) s
      else if token = "then" then Tok_Then :: tok (pos + length token) s
      else if token = "else" then Tok_Else :: tok (pos + length token) s
      else if token = "rec" then Tok_Not :: tok (pos + length token) s
      else if token = "if" then Tok_If :: tok (pos + length token) s
      else if token = "in" then Tok_In :: tok (pos + length token) s
      else Tok_ID token :: tok (pos + length token) s
    else if Str.string_match re_intt s pos then
      let token = Str.matched_string s in
    ...
    else if Str.string_match re_rprn s pos then Tok_RParen :: tok (pos + 1) s
    else if Str.string_match re_lprn s pos then Tok_LParen :: tok (pos + 1) s
    else if Str.string_match re_eq s pos then Tok_Equal :: tok (pos + 1) s
    else if Str.string_match re_neq s pos then Tok_NotEqual :: tok (pos + 2) s
    else if Str.string_match re_grtreq s pos then
      Tok_GreaterEqual :: tok (pos + 2) s
    else if Str.string_match re_lseq s pos then Tok_LessEqual :: tok (pos + 2) s
    else if Str.string_match re_grtr s pos then Tok_Greater :: tok (pos + 1) s
    else if Str.string_match re_ls s pos then Tok_Less :: tok (pos + 1) s
    ...
    else if Str.string_match re_dblsmi s pos then
