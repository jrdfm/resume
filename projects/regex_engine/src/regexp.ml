...
let g_fs ls = match ls with
| h::t -> h
| [] -> (-1);;


let un a b = 
  let f,s = fresh(),fresh() in
  {
  sigma = union a.sigma b.sigma;
  qs = union(union a.qs b.qs) [s;f];
  q0 = s;fs = [f];
  delta = union (union a.delta b.delta ) [(s,None,a.q0);(s,None,b.q0);(g_fs(a.fs),None,f);(g_fs(b.fs),None,f)];
  }
...
let ct a b = 
  ...
  qs = union a.qs b.qs;
  q0 = a.q0;fs = b.fs;
  delta = union (union a.delta b.delta ) [(g_fs a.fs,None,b.q0)];
  }
...
let str a = 
  let f,s = fresh(),fresh() in
    ...
    qs = union a.qs [s;f];
    q0 = a.q0;fs = [f];
    delta = union a.delta [(s,None,a.q0);(g_fs a.fs,None,f);(s,None,f);(f,None,s)];
    }

let rec regexp_to_nfa (regexp: regexp_t) : (int, char) nfa_t = match regexp with
  | Empty_String -> let f,s = fresh(),fresh() in
                  {sigma = [];qs = [s;f];q0 = s;fs = [f];delta = [(s,None,f)]}
  | Char (a) -> let f,s = fresh(),fresh() in
              {sigma = [a];qs = [s;f];q0 = s;fs = [f];delta = [(s,Some a,f)]}
  | Union (a, b) -> un (regexp_to_nfa a) (regexp_to_nfa b)
  | Concat (a, b) -> ct (regexp_to_nfa a) (regexp_to_nfa b)
  | Star a -> str (regexp_to_nfa a) ;;
